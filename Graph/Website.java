...
public class Website {
    ...
    double similarity;
    ArrayList<Website> connectedSites;
    ...
    public String getNodeName(){
        String[] pieces = this.URL.split("https://en.wikipedia.org/wiki/");
        return pieces[1];
    }
    ...
    public static ArrayList<String> getAllHeadings(ArrayList<Website> w){
    ...
    public static ArrayList<String> getAllWords(ArrayList<Website> w){
    ...
    //TODO: repetitivly getting scores see if can do this will less iteration / make faster ???
    // persist some of this please???
    public void similarity(Website w2, ArrayList<Website> w, ArrayList<String> allWords, ArrayList<String> allHeadings) {
        ...
        for (String word : allWords) {
            document1[counter] = tf(this, word) * idf(w, word) *.20;
            // System.out.println("COUNTER SPOT" + document1[counter]);
            counter += 1;
        }

        for (String word : allHeadings) {
            document1[counter] = tf(this, word) * idf(w, word) *.80;
            // System.out.println("COUNTER SPOT" + document1[counter]);
            counter += 1;
        }
   ...
   /* public void setSimilarity(ArrayList<Website> w, ArrayList<String> allWords, ArrayList<String> allHeadings) {
        // go through all the words in our website
        double similarity;
        double dotproduct = 0;
        double bottom = 0;
 ...
 */
    ...
    public Website getPersistent(String URL) throws Exception {
        // Get contents from local file

        // "hash" the url to get the key
        String filePath;
        if (URL.startsWith("C:") && URL.endsWith(".txt")) {
            // C:\Users\Person\IdeaProjects\Graph\httpsenwikipediaorgwikiFifthThirdArena.txt
            filePath = URL;
        } else if (URL.endsWith(".txt")) {
            // httpsenwikipediaorgwikiGuido_Karptxt
            filePath = System.getProperty("user.dir") + File.separator + getCutURL();
        } else {
    ...
    public void getConnectedSites() throws IOException {
    ...
    public void makePersistent() throws IOException {
        ...
        String filePath = System.getProperty("user.dir") + File.separator + getCutURL() + ".txt";
        File file = new File(filePath);
